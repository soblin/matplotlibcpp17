...
If you want to see the demo with `plt.show()`, add `-DUSE_GUI=1` (by default it is `0`). Otherwise the executables will `plt.savefig()` to each source directory. Then `make <gallery directory name>` runs all executables under that directory.
